    ...
    <div class="min-h-screen flex flex-col">
        <UContainer class="sticky top-0 pt-4 inset-x-0 z-10">
            <UCard class="rounded-full bg-default/40 backdrop-blur-lg" :ui="{ body: 'flex items-center justify-between gap-1 md:gap-2 p-2!' }">
                <div class="flex gap-2 items-center md:ms-2">
                ...
                <div class="flex justify-end gap-2 p-4 text-sm max-md:hidden">
                    <SwitchLocalePathLink locale="id">ID</SwitchLocalePathLink>
                    <SwitchLocalePathLink locale="en">EN</SwitchLocalePathLink>
                </div>
                <ColorModeButton />
                <UButton :icon="isMenuOpen ? 'i-lucide-x' : 'i-lucide-menu'" variant="ghost" class="cursor-pointer lg:hidden rounded-full" color="neutral" @click="toggleMenu" />
                <UButton label="Book a call" color="neutral" size="lg" class="rounded-full hidden lg:flex" icon="i-lucide-phone" :href="`https://wa.me/${config.public.owner.phone}`" />
            </UCard>
            <Transition name="fade">
                <UCard v-if="isMenuOpen" class="mt-2 rounded-2xl bg-default/50 backdrop-blur-lg lg:hidden fixed inset-x-4" :ui="{ body: 'p-4' }">
                    <div class="flex flex-col gap-3 w-full">
                        <UButton v-for="nav in navigations" :key="nav.label" :to="nav.to" variant="ghost" class="cursor-pointer justify-start text-left" @click="closeMenu" color="neutral">
                            {{ nav.label }}
                        </UButton>
                        <div class="flex justify-between">
                            <UButton label="Book a call" color="neutral" size="lg" class="rounded-full" icon="i-lucide-phone" :href="`https://wa.me/${config.public.owner.phone}`" />
